...
def Dockerdeploy(env_Name, host_Port) {
    ...
    echo "***** deploying docker in $env_Name env *****"
            ...
            script {
                ...
                sh "sshpass -p ${PASSWORD} -v ssh -o StrictHostKeyChecking=no ${USERNAME}@35.224.75.2 docker pull ${env.DOCKER_HUB}/${env.SERVICE_NAME}:${GIT_COMMIT}"
                try {
                    // stop the container 
                    echo "stopping the container"
                    sh "sshpass -p ${PASSWORD} -v ssh -o StrictHostKeyChecking=no ${USERNAME}@35.224.75.2 docker stop ${env.SERVICE_NAME}-$env_Name"

                    // remove the container
                    echo "removing the container"
                    sh "sshpass -p ${PASSWORD} -v ssh -o StrictHostKeyChecking=no ${USERNAME}@35.224.75.2 docker rm ${env.SERVICE_NAME}-$env_Name"
                ...
                // docker create a conatainer
                   ...
                   sh "sshpass -p ${PASSWORD} -v ssh -o StrictHostKeyChecking=no ${USERNAME}@35.224.75.2 docker run -d -p $host_Port:8761 --name ${env.SERVICE_NAME}-$env_Name ${env.DOCKER_HUB}/${env.SERVICE_NAME}:${GIT_COMMIT}"
